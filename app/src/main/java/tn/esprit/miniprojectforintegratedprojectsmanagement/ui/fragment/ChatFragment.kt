...
import tn.esprit.miniprojectforintegratedprojectsmanagement.databinding.FragmentChatBinding
...
import tn.esprit.miniprojectforintegratedprojectsmanagement.ui.activity.EntranceActivity
...
class ChatFragment : Fragment() {
    ...
    private fun getChatGroupeList(): MutableList<Group> {
// create a group list
        ...
        services.getgroups().enqueue(object : Callback<List<Group>> {
            override fun onResponse(call: Call<List<Group>>, response: Response<List<Group>>) {
                if (response.isSuccessful) {
                    ...
                    ChatRecyclerView = binding.ChatRecyclerGroupView

                    ChatRecyclerView.adapter = GroupAdapter(groups?.toMutableList()!!, onItemClick = {
                        println("aaaa "+ it.id)
                        //navigate to chat entrance activity then to the chat room
                        val intentchatentrance = Intent(requireContext(),EntranceActivity::class.java)
                        intentchatentrance.putExtra("room_id",it.id)
                            .apply {  }
                        startActivity(intentchatentrance)
